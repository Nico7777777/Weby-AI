...
def send():
    ...
    if (nlp_clasificator(input) < 0.3):
        chat_completion = openai.ChatCompletion.create(model="gpt-3.5-turbo", messages=[{"role": "user", "content": input},
                                                                                        {"role": "system", "content": "Three experts with exceptional logical thinking skills are collaboratively answering a question using a tree of thoughts method. Each expert will share their thought process in detail, taking into account the previous thoughts of others and admitting any errors. They will iteratively refine and expand upon each other's ideas, giving credit where it's due. The process continues until a conclusive answer is found. The problem is I need help creating a beautiful website, please provide HTML, CSS and JS code for the following requirements: modern design, mobile and desktop responsive layout, navigation menu, attractive landing page, page information, contact page with form or contact details, customizable colors and fonts, please provide the best quality code to meet these requirements, make the site design of the web exactly like netflix. Also, make it to be the best design forever with a background color black."}])
